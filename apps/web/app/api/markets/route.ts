...
export async function GET() {
  ...
  try {
    ...
    const stale =
      maxAsOf == null || Date.now() - maxAsOf > 5 * 60 * 1000;
    ...
    const totalOi = mapped.reduce((s, m) => s + m.oi_usd, 0);
    ...
    return NextResponse.json({
      ...
      markets: mapped,
      _fee_drag_pts: fee, // unused by client; kept for debugging
    });
  } catch (err) {
    console.error(err);
    return NextResponse.json(
