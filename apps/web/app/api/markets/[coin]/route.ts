...
export async function GET(
  ...
  try {
    ...
    // 30d daily APR history from funding_ticks
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      // weekday in UTC noon ≈ correct for ET date key
      const dow = d.getUTCDay(); // 0 Sun
      if (dow === 6) {
        // Saturday — pair with next (Sun) if present
        const idx = 29 - i;
        if (i > 0) weekendIdx.push([idx, idx + 1]);
      }
    }
